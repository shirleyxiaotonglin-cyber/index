/**
 * 会议纪要 OpenRouter 解析（供 parse-meeting.js 与 workgraph-insights.js 共用）。
 * 禁止在 serverless 内再 fetch 同源 /api/ai/parse-meeting，否则 Vercel 可能判定无限循环（508）。
 */
...
/**
 * @param {string} text
 * @returns {Promise<{ ok: true, parsed: unknown } | { ok: false, status: number, error: string, detail?: string }>}
 */
async function runMeetingParseFromText(text) {
  ...
  for (round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (mi = 0; mi < models.length; mi++) {
      ...
      if (r.status === 401) {
        return { ok: false, status: 502, error: "OpenRouter 请求失败", detail: raw.slice(0, 400) };
      }
      if (!r.ok) {
      ...
      if (pr.kind === "ok") {
        ...
        try {
          ...
          return { ok: false, status: 502, error: "模型输出不是合法 JSON", detail: pr.content.slice(0, 240) };
        }
...
module.exports = { runMeetingParseFromText };
