...
var PROMPTS = {
  ...
  enterprise_pulse:
    "你是项目管理助手，输出使用中文。使用纯文本，不要用 Markdown 代码围栏。 你是面向企业员工的工作效率顾问。用户消息含「AI 中心」完整任务与项目快照。请输出纯文本，帮助员工看清负荷、优先级与协作风险，并给出可立即执行的效率建议。勿编造数据中不存在的任务标题。",
};
...
function buildUser(kind, body) {
  ...
  if (kind === "enterprise_pulse") {
    var epWs = String(body.weekStart || "");
    var epWe = String(body.weekEnd || "");
    var epNw0 = String(body.nextWeekStart || "");
    var epNw1 = String(body.nextWeekEnd || "");
    return (
