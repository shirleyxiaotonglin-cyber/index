import { normalizeMeetingParsedEnhanced } from "@/lib/meeting-parse-postprocess";
/**
...
1) 只能输出合法 JSON；字段类型稳定，不能省略 tasks 字段。
2) 优先级映射：紧急/最高/阻塞=>P0，高=>P1，中=>P2，低=>P3；不确定用 P2。
3) 截止日期尽量输出 YYYY-MM-DD；相对日期（如 明天/本周五/下周三）按当前时间推断，无法确定输出 ""。
4) 负责人与协作人识别：支持“由A负责，B协作”“A（主），B/C”；不确定时保守留空，不要编造。
5) 不要虚构项目名、阶段、负责人、日期；缺失信息宁可空值。
6) 对同义重复任务可合并，保持 title 清晰简短，description 保留关键上下文。`;

/**
 * 默认使用 DeepSeek V3.2（非 free），减少免费池限流带来的不稳定。
 * 需要固定其它模型时再设 OPENROUTER_MEETING_MODEL。
 */
const DEFAULT_MEETING_MODEL = "deepseek/deepseek-v3.2";
  ...
  for (let round = 0; round < MAX_RATE_LIMIT_ROUNDS; round++) {
    ...
    for (const model of models) {
      ...
      if (parsed.kind === "message") {
        ...
        return { ok: true, parsed: normalizeMeetingParsedEnhanced(out, new Date()) };
      }
