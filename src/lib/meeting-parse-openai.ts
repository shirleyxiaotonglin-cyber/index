...
export async function parseMeetingWithOpenAI(text: string): Promise<MeetingParseResult> {
  const key = process.env.OPENROUTER_API_KEY;
  if (!key?.trim()) {
    return { ok: false, error: "服务器未配置 OPENROUTER_API_KEY" };
  }
  const model = process.env.OPENROUTER_MEETING_MODEL || "gpt-4o-mini";
  const body = {
    model,
    temperature: 0.2,
  ...
  try {
    ...
    return { ok: false, error: "Openrouter 返回非 JSON", detail: raw.slice(0, 200) };
  }

  const content = (data as { choices?: { message?: { content?: string } }[] })?.choices?.[0]?.message
    ?.content;
  if (!content || typeof content !== "string") {
    return { ok: false, error: "Openrouter 未返回内容", detail: raw.slice(0, 200) };
  }
