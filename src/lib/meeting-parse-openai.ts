...
/** 主模型 429 / 上游限流时依次尝试（不同路由，缓解 Venice 等单路限流） */
const MEETING_FALLBACK_MODELS = ["openrouter/free", "meta-llama/llama-3.2-3b-instruct:free"] as const;
...
function uniqModelOrder(primary: string): string[] {
  ...
  for (const m of [primary, ...MEETING_FALLBACK_MODELS]) {
    if (!seen.has(m)) {
      seen.add(m);
      out.push(m);
    }
...
function parseChatCompletionJson(raw: string): ChatJson {
  ...
  if (err && !hasChoices) {
    ...
    if (
      err.code === 429 ||
      /rate-?limit|temporarily rate-limited|429/i.test(msg) ||
